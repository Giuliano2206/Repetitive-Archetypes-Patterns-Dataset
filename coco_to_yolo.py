...
parser.add_argument(
    ...
    const=True,
    help=(
        'True if COCO format has segments instead of bounding boxes'
        'False if COCO format has bounding boxes instead of'
        'segments'
    )
)
...
def make_dirs(dir='datasets/yolo_datasets/'):
    # Create folders
    dir = Path(dir)
    if dir.exists():
...
def convert_coco_json(json_files, use_segments=False):
    ...
    # Import json
    for json_file in sorted(list(map(Path, json_files))):
        dir_json = json_file.parent
        fn = Path(save_dir) / 'labels' / dir_json.name
        fn.mkdir()
